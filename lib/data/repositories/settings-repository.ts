...
export class SettingsRepository implements ISettingsRepository {
    ...
    async update(settings: Partial<Settings>): Promise<Settings> {
        ...
        const updated: Settings = {
            ...
            payment: {
                ...current.payment,
                ...(settings.payment ?? {}),
                enabledGateways: settings.payment?.enabledGateways ??
            ...
            certificate: {
                ...
                customTemplates: settings.certificate?.customTemplates ??
            ...
            system: {
                ...current.system,
                ...(settings.system ?? {}),
                allowedIps: settings.system?.allowedIps ??
    ...
    async updatePaymentSettings(settings: Partial<Settings['payment']>): Promise<Settings['payment']> {
        const current = await this.findAll();
        const updated = await this.update({
            payment: {
                ...current.payment,
    ...
    private cloneDefaults(): Settings {
        return {
            ...
            certificate: {
                ...
                customTemplates: DEFAULT_SETTINGS.certificate.customTemplates
                    ? DEFAULT_SETTINGS.certificate.customTemplates.map(template => ({
                        ...template,
                    }))
                    : undefined,
            },
