...
async function ensureHydrated() {
    ...
    if (!hydration) {
        hydration = (async () => {
            const saved = await browserDbGet<Schedule[]>('schedules');

            if (saved !== null) {
                schedules = saved.map(normalizeSchedule);
            }
...
async function persist() {
    await browserDbSet('schedules', schedules);
}

const waitlists = new Map<string, string[]>();

function matches(s: Schedule, q?: ScheduleQuery) {
    const f = q?.filter;

    if (!f) return true;

    if (f.courseId && s.courseId !== f.courseId)
        return false;
    ...
    if (typeof f.published === 'boolean' && s.published !== f.published)
        return false;

    if (f.available && s.currentParticipants >= s.maxParticipants)
        return false;

    if (f.startDateFrom && s.startDate < f.startDateFrom)
        return false;

    if (f.startDateTo && s.startDate > f.startDateTo)
        return false;

    if (
        f.searchQuery &&
        !`${s.title} ${s.courseTitle} ${s.city ?? ''}`
            .toLowerCase()
            .includes(f.searchQuery.trim().toLowerCase())
    )
        return false;
...
function sorted(list: Schedule[], q?: ScheduleQuery) {
    const r = [...list];
    const ord = q?.order ?? 'asc';
    ...
    r.sort((a, b) => {
        ...
        if (q?.sort === 'createdAt')
            ...
            n = a.startDate.getTime() - b.startDate.getTime();
...
export class ScheduleRepository implements IScheduleRepository {
    ...
    async findAll(q?: ScheduleQuery) {
        await ensureHydrated();

        return sorted(
            schedules.filter((s: Schedule) => matches(s, q)),
            q
        );
    }
    ...
    async findByCourseId(id: string, q?: ScheduleQuery) {
        await ensureHydrated();

        return sorted(
            schedules.filter(
                s => s.courseId === id && matches(s, q)
            ),
            q
        );
    }

    async findAvailableSchedules(id: string) {
        await ensureHydrated();
    ...
    async findUpcomingSchedules(q?: ScheduleQuery) {
        await ensureHydrated();
    ...
    async findSessionsByScheduleId(id: string) {
        await ensureHydrated();

        return (
            schedules.find(s => s.id === id)?.sessions ?? []
        );
    }

    async createSession(scheduleId: string, input: any) {
        await ensureHydrated();

        const s = await this.findById(scheduleId);
    ...
    async updateSession(id: string, input: any) {
        await ensureHydrated();
    ...
    async updateParticipantCount(id: string, inc: number) {
        await ensureHydrated();
        ...
        s.currentParticipants = Math.max(
            0,
            s.currentParticipants + inc
        );
    ...
    async addToWaitlist(
        ...
        if (!list.includes(traineeId))
            list.push(traineeId);

        waitlists.set(id, list);
