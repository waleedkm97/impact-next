import type { Course, CourseLesson, CourseQuery, CourseFilter, LessonQuestion, } from '@/types/course';
...
function matches(course: Course, filter?: CourseFilter) {
    ...
    if (filter.delivery && course.delivery !== filter.delivery)
        return false;
    if (filter.categoryId && course.categoryId !== filter.categoryId)
        return false;
    if (filter.city && !(course.cities ?? []).includes(filter.city))
        return false;
    if (typeof filter.minPrice === 'number' && course.price < filter.minPrice)
        return false;
    if (typeof filter.maxPrice === 'number' && course.price > filter.maxPrice)
        return false;
    if (typeof filter.featured === 'boolean' && course.featured !== filter.featured)
        return false;
    if (typeof filter.published === 'boolean' && course.published !== filter.published)
        return false;
...
export class CourseRepository implements ICourseRepository {
    ...
    async findAll(query?: CourseQuery) {
        ...
        result.sort((a, b) => {
            let n = 0;
            if (query?.sort === 'title')
                n = a.title.localeCompare(b.title, 'ar');
            else if (query?.sort === 'price')
                n = a.price - b.price;
            else if (query?.sort === 'popularity')
                n = Number(b.featured) - Number(a.featured);
            else
    ...
    async create(input: Omit<Course, 'id' | 'createdAt' | 'updatedAt'>) {
        await ensureHydrated();
        const now = new Date();
    ...
    async findLessonById(lessonId: string) { await ensureHydrated(); for (const c of courses) {
    ...
    async createLesson(courseId: string, input: Omit<CourseLesson, 'id' | 'courseId' | 'createdAt' | 'updatedAt'>) {
        await ensureHydrated();
    ...
    async updateLesson(lessonId: string, updates: Partial<CourseLesson>) {
        await ensureHydrated();
    ...
    async deleteLesson(lessonId: string) {
        ...
        for (const course of courses) {
            ...
            if (course.lessons.length !== before) {
                course.videosCount = course.lessons.filter((l) => l.type === 'video').length;
                course.updatedAt = new Date();
                await persist();
                return;
