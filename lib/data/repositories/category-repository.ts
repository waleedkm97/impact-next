...
async function ensureHydrated() { if (hydrated)
    ...
    hydration = (async () => { const saved = await browserDbGet<ExtraCategory[]>('categories'); if (saved !== null)
        categories = saved; hydrated = true; })().catch(() => { hydrated = true; });
} await hydration; }
async function persist() { await browserDbSet('categories', categories); }
function matches(c: ExtraCategory, q?: CategoryQuery) { const f = q?.filter; if (!f)
    return true; if (typeof f.published === 'boolean' && c.published !== f.published)
    return false; if (f.searchQuery && !`${c.name} ${c.description ?? ''} ${c.slug ?? ''}`.toLowerCase().includes(f.searchQuery.trim().toLowerCase()))
    return false; return true; }
export class CategoryRepository implements ICategoryRepository {
    async findById(id: string) { await ensureHydrated(); return categories.find(c => c.id === id) ?? null; }
    async findBySlug(slug: string) { await ensureHydrated(); const s = slugify(slug); return categories.find(c => c.slug === s || slugify(c.name) === s) ?? null; }
    async findAll(q?: CategoryQuery) { await ensureHydrated(); let r = categories.filter(c => matches(c, q)); r.sort((a, b) => a.name.localeCompare(b.name, 'ar')); if (q?.order === 'desc')
    ...
    async update(id: string, input: any) { await ensureHydrated(); const i = categories.findIndex(c => c.id === id); if (i < 0)
        throw new Error('Category not found'); categories[i] = { ...categories[i], ...input, id, slug: input.slug ?? categories[i].slug ?? slugify(input.name ?? categories[i].name), updatedAt: new Date() }; await persist(); return categories[i]; }
    ...
    async reorder(ids: string[]) { await ensureHydrated(); ids.forEach((id, i) => { const c = categories.find(x => x.id === id); if (c)
        c.order = i; }); categories.sort((a, b) => (a.order ?? 0) - (b.order ?? 0)); await persist(); }
}
