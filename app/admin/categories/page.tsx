...
export default function CategoriesPage() {
    ...
    async function load() { const [c, cs] = await Promise.all([categoryRepository.findAll({ sort: 'name', order: 'asc' }), courseRepository.findAll()]); setCategories(c as any); setCourses(cs); }
    useEffect(() => { void load(); }, []);
    ...
    async function save(e: React.FormEvent) { e.preventDefault(); if (!name.trim())
        ...
        await categoryRepository.update(editing, data);
    else
        await categoryRepository.create(data as any); setOpen(false); await load(); }
    async function remove() { if (!deleteId)
        ...
        setDeleteId(null);
        return;
    } await categoryRepository.delete(deleteId); setDeleteId(null); await load(); }
    async function toggle(c: Category) { await categoryRepository.update(c.id, { published: c.published === false }); await load(); }
