...
const emptySchedule: ScheduleForm = { delivery: 'in-person', startDate: '', endDate: '', startTime: '09:00', endTime: '14:00', city: 'الرياض', location: '', meeting: '', max: '20', price: '' };
function priceFor(city: string, base: number) { if (city === 'أونلاين')
...
function sundays(year: number) { const d = new Date(`${year}-01-01T00:00:00`); while (d.getDay() !== 0)
...
export default function ProgramsAdmin() {
    ...
    function editSchedule(s: Schedule) { setEditingSchedule(s.id); setSf({ delivery: s.city === 'أونلاين' ? 'online' : 'in-person', startDate: s.startDate.toISOString().slice(0, 10), endDate: s.endDate.toISOString().slice(0, 10), startTime: s.startTime, endTime: s.endTime, city: s.city ?? 'الرياض', location: s.location ?? '', meeting: s.onlineMeetingLink ?? '', max: String(s.maxParticipants), price: String(s.price ?? '') }); }
    async function saveSchedule(e: React.FormEvent) { e.preventDefault(); if (!programForSchedule || !sf.startDate)
        return; const online = sf.delivery === 'online'; const data: any = { courseId: programForSchedule.id, courseTitle: programForSchedule.title, title: programForSchedule.title, description: programForSchedule.description, startDate: asDate(sf.startDate), endDate: asDate(sf.endDate || sf.startDate), startTime: sf.startTime, endTime: sf.endTime, city: online ? 'أونلاين' : sf.city, location: online ? undefined : sf.location || undefined, onlineMeetingLink: online ? sf.meeting || undefined : undefined, maxParticipants: Number(sf.max) || 20, currentParticipants: 0, price: Number(sf.price) || priceFor(online ? 'أونلاين' : sf.city, Number(programForSchedule.price)), currency: 'SAR', instructorName: programForSchedule.trainer?.name, status: 'available', published: programForSchedule.published, allowWaitlist: true, requireConfirmation: false, recurrence: 'once' }; if (editingSchedule)
    ...
    async function generate(year: number) { if (!programForSchedule)
        return; const existing = await scheduleRepository.findByCourseId(programForSchedule.id); const used = new Set(existing.map(s => s.startDate.toISOString().slice(0, 10) + '|' + (s.city ?? ''))); const dates = sundays(year); let made = 0; for (let i = 0; i < dates.length; i++) {
        const city = i % 11 === 10 ? 'أونلاين' : cities[i % 10];
        const key = dates[i] + '|' + city;
        if (used.has(key))
            continue;
        const online = city === 'أونلاين';
        await scheduleRepository.create({ courseId: programForSchedule.id, courseTitle: programForSchedule.title, title: programForSchedule.title, description: programForSchedule.description, startDate: asDate(dates[i]), endDate: asDate(dates[i]), startTime: '09:00', endTime: online ? '12:00' : '14:00', city, location: undefined, onlineMeetingLink: undefined, maxParticipants: 20, currentParticipants: 0, waitlistMax: 10, currentWaitlist: 0, price: priceFor(city, Number(programForSchedule.price)), currency: 'SAR', instructorName: programForSchedule.trainer?.name, status: 'available', published: programForSchedule.published, allowWaitlist: true, requireConfirmation: false, recurrence: 'once' });
        used.add(key);
        made++;
    } setSchedules(await scheduleRepository.findByCourseId(programForSchedule.id)); alert(made ? `تم إنشاء ${made} موعدًا لعام ${year}.` : 'لا توجد مواعيد جديدة لهذا العام.'); }
    async function generateAll(year: number) { if (!programs.length)
        return alert('لا توجد برامج تدريبية.'); if (!confirm(`سيتم جدولة جميع البرامج لعام ${year}. هل تريد المتابعة؟`))
        return; let total = 0; for (const p of programs) {
        const existing = await scheduleRepository.findByCourseId(p.id);
        const used = new Set(existing.map(s => s.startDate.toISOString().slice(0, 10) + '|' + (s.city ?? '')));
        for (let i = 0; i < sundays(year).length; i++) {
            const date = sundays(year)[i];
            const city = i % 11 === 10 ? 'أونلاين' : cities[i % 10];
            if (used.has(date + '|' + city))
                continue;
            const online = city === 'أونلاين';
            await scheduleRepository.create({ courseId: p.id, courseTitle: p.title, title: p.title, description: p.description, startDate: asDate(date), endDate: asDate(date), startTime: '09:00', endTime: online ? '12:00' : '14:00', city, location: undefined, onlineMeetingLink: undefined, maxParticipants: 20, currentParticipants: 0, waitlistMax: 10, currentWaitlist: 0, price: priceFor(city, Number(p.price)), currency: 'SAR', instructorName: p.trainer?.name, status: 'available', published: p.published, allowWaitlist: true, requireConfirmation: false, recurrence: 'once' });
            used.add(date + '|' + city);
            total++;
        }
    } alert(`تم إنشاء ${total} موعدًا لعام ${year} لجميع البرامج.`); if (programForSchedule)
        setSchedules(await scheduleRepository.findByCourseId(programForSchedule.id)); }
 ...
 {scheduleOpen && programForSchedule && <div className="admin-modal-backdrop"><div className="admin-modal" style={{ maxWidth: 1050 }}><div className="admin-modal-header"><div><h2>إدارة المواعيد — {programForSchedule.title}</h2><p style={{ margin: 4, color: '#6b7890', fontSize: 12 }}>الحضور والموقع أو رابط Zoom يضاف للمشارك بعد التسجيل.</p></div><button className="admin-modal-close" onClick={closeSchedules}>×</button></div><div className="admin-modal-body"><div className="admin-actions" style={{ marginBottom: 15 }}><button className="admin-btn admin-btn-gold" onClick={() => void generate(2026)}>جدولة هذا البرنامج 2026</button><button className="admin-btn admin-btn-gold" onClick={() => void generate(2027)}>جدولة هذا البرنامج 2027</button><button className="admin-btn admin-btn-light" onClick={() => { setEditingSchedule(null); setSf(emptySchedule); }}>+ إضافة موعد يدوي</button></div><form onSubmit={saveSchedule} className="admin-card" style={{ padding: 15, marginBottom: 15 }}><div className="admin-form-grid"><Field label="نوع التنفيذ"><select className="admin-select" value={sf.delivery} onChange={e => setSf({ ...sf, delivery: e.target.value as any, endTime: e.target.value === 'online' ? '12:00' : '14:00', city: e.target.value === 'online' ? 'أونلاين' : 'الرياض' })}><option value="in-person">حضوري</option><option value="online">أونلاين</option></select></Field><Field label="تاريخ البداية"><input className="admin-input" type="date" required value={sf.startDate} onChange={e => setSf({ ...sf, startDate: e.target.value, endDate: sf.endDate || e.target.value })}/></Field><Field label="تاريخ النهاية"><input className="admin-input" type="date" value={sf.endDate} onChange={e => setSf({ ...sf, endDate: e.target.value })}/></Field><Field label="المدينة"><select className="admin-select" disabled={sf.delivery === 'online'} value={sf.delivery === 'online' ? 'أونلاين' : sf.city} onChange={e => setSf({ ...sf, city: e.target.value })}><option value="الرياض">الرياض</option>{cities.slice(1).map(c => <option key={c} value={c}>{c}</option>)}</select></Field><Field label="وقت البداية"><input className="admin-input" type="time" value={sf.startTime} onChange={e => setSf({ ...sf, startTime: e.target.value })}/></Field><Field label="وقت النهاية"><input className="admin-input" type="time" value={sf.endTime} onChange={e => setSf({ ...sf, endTime: e.target.value })}/></Field><Field label="السعر"><input className="admin-input" type="number" min="0" value={sf.price || priceFor(sf.delivery === 'online' ? 'أونلاين' : sf.city, Number(programForSchedule.price))} onChange={e => setSf({ ...sf, price: e.target.value })}/></Field><Field label="الحد الأقصى"><input className="admin-input" type="number" min="1" value={sf.max} onChange={e => setSf({ ...sf, max: e.target.value })}/></Field>{sf.delivery === 'in-person' ? <Field label="الموقع"><input className="admin-input" placeholder="يظهر للمشارك بعد التسجيل" value={sf.location} onChange={e => setSf({ ...sf, location: e.target.value })}/></Field> : <Field label="رابط Zoom"><input className="admin-input" placeholder="يظهر للمشارك بعد التسجيل" value={sf.meeting} onChange={e => setSf({ ...sf, meeting: e.target.value })}/></Field>}</div><div className="admin-modal-footer" style={{ padding: '14px 0 0', border: 0 }}><button className="admin-btn admin-btn-primary">{editingSchedule ? 'حفظ تعديل الموعد' : 'إضافة الموعد'}</button>{editingSchedule && <button type="button" className="admin-btn admin-btn-light" onClick={() => { setEditingSchedule(null); setSf(emptySchedule); }}>إلغاء التعديل</button>}</div></form><div className="admin-table-card"><table className="admin-table"><thead><tr><th>التاريخ</th><th>التنفيذ</th><th>المدينة</th><th>الوقت</th><th>السعر</th><th>الإجراءات</th></tr></thead><tbody>{schedules.sort((a, b) => a.startDate.getTime() - b.startDate.getTime()).map(s => <tr key={s.id}><td>{s.startDate.toLocaleDateString('ar-SA')}</td><td>{s.city === 'أونلاين' ? 'أونلاين' : 'حضوري'}</td><td>{s.city || '—'}</td><td>{s.startTime} - {s.endTime}</td><td>{Number(s.price || 0).toLocaleString('ar-SA')} SAR</td><td><button className="admin-btn admin-btn-light" onClick={() => editSchedule(s)}>تعديل</button> <button className="admin-btn admin-btn-danger" onClick={() => void deleteSchedule(s.id)}>حذف</button></td></tr>)}{schedules.length === 0 && <tr><td colSpan={6} style={{ textAlign: 'center', padding: 25, color: '#6b7890' }}>لا توجد مواعيد حتى الآن.</td></tr>}</tbody></table></div></div><div className="admin-modal-footer"><button className="admin-btn admin-btn-light" onClick={closeSchedules}>إغلاق</button></div></div></div>}
 </main>;
}
