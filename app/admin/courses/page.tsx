...
function toForm(c: Course): Form {
    return {
        title: c.title,
        categoryId: c.categoryId ?? '',
        price: String(c.price ?? ''),
        oldPrice: c.oldPrice ? String(c.oldPrice) : '',
        ...
        pre: c.preAssessmentEnabled !== false,
        post: c.postAssessmentEnabled !== false,
        evaluation: c.courseEvaluationEnabled !== false,
    };
}
...
export default function CoursesAdmin() {
    ...
    function readMaterialFile(file: File) {
        if (file.type !== 'application/pdf') {
            return alert('يرجى اختيار ملف PDF فقط.');
        }

        if (file.size > 8 * 1024 * 1024) {
            return alert('حجم ملف PDF يجب ألا يتجاوز 8MB.');
        }

        const reader = new FileReader();

        reader.onload = () =>
            set('materialUrl', String(reader.result || ''));
    ...
    async function save(e: React.FormEvent) {
        ...
        const payload: any = {
            ...
            lessons: editing
                ? courses.find((c) => c.id === editing)?.lessons ?? []
                : [],
            ...
            // مهم:
            // لا نرسل assessments: []
            // حتى لا يؤدي تعديل الدورة إلى مسح تقييماتها الموجودة.
            ...
            createdAt: editing
                ? courses.find((c) => c.id === editing)?.createdAt ??
                  new Date()
                : new Date(),
        };
        ...
        <main className="admin-page" dir="rtl">
            <header className="admin-page-header">
                <div>
                    ...
                    <p>
                        الدورات المسجلة والمحتوى الرقمي المتاح للتعلم
                        الذاتي.
                    </p>
                </div>
            ...
            <section className="admin-card-grid">
                {visible.length === 0 ? (
                    ...
                    >
                        <strong>لا توجد دورات</strong>

                        <span>
                            ابدأ بإضافة دورة مسجلة جديدة.
                        </span>
                    </div>
                ) : (
                    visible.map((c) => {
                                ...
                                <div className="admin-course-body">
                                    ...
                                    <div className="admin-card-actions">
                                        ...
                                        <Link
                                            ...
                                            href={`/admin/courses/${c.id}/assessments`}
                                        >
                                            التقييمات
                                        </Link>
