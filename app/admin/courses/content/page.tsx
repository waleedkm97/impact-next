...
export default function RecordedCourseContentPage() {
    ...
    if (loading)
        return <main dir="rtl" style={{ padding: 32 }}>جاري التحميل...</main>;
    if (!course)
        return <main dir="rtl" style={{ padding: 32 }}>الدورة غير موجودة.</main>;
    return (<main dir="rtl" style={{ padding: 32, maxWidth: 1200, margin: '0 auto' }}>
      ...
      <section style={{ display: 'grid', gap: 12 }}>
        {lessons.length === 0 ? <p>لا يوجد محتوى حتى الآن.</p> : lessons.map((lesson, index) => (<article key={lesson.id} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, padding: 18, display: 'flex', justifyContent: 'space-between', gap: 20 }}>
