...
function CheckoutContent() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!sqlTraineeId) {
        throw new Error('تعذر تحديد حساب المتدرب.');
      }

      const enrollmentsResponse = await fetch(
        `/api/enrollments?traineeId=${encodeURIComponent(sqlTraineeId)}`,
        {
          cache: 'no-store',
        },
      );
      ...
      if (enrollmentsResponse.ok) {
        ...
        if (existing) {
          router.push(
            `/course-learning?id=${encodeURIComponent(course.id)}`,
          );
          return;
        }
      }
      ...
      if (!orderResponse.ok) {
        throw new Error(
          orderResult?.error || 'تعذر إنشاء الطلب.',
        );
      }

      setMessage(
        'تم إرسال طلب الشراء. سيتم تفعيل الدورة بعد تأكيد الدفع.',
      );
    } catch {
  ...
  if (!user) {
    return (
      <main dir="rtl" className="container mx-auto px-6 py-12">
        ...
        <Link
          href={`/login?next=/checkout?id=${encodeURIComponent(course.id)}`}
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_350px]">
        <section className="card" style={{ padding: 24 }}>
          ...
          <div className="checkout-summary-list">
            ...
            <div>
              <span>الجوال</span>
              <strong>{user.contact.phone || '—'}</strong>
            </div>
          </div>
