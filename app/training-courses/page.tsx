...
export default function TrainingProgram() {
  ...
  const cities = useMemo(
    () =>
      allowedCities.filter((city) =>
        schedules.some(
          (schedule) => (schedule.city || onlineCity) === city,
        ),
      ),
    [schedules],
  );
        ...
        <section>
          ...
          <article className="course-details-section">
            ...
            {outline.length ? (
              ...
              <p>
                سيتم عرض المحاور عند إضافتها إلى الدورة.
              </p>
            )}
          </article>
        </section>
          ...
          <div className="training-schedule-list">
            {visibleSchedules.length === 0 ? (
              <div className="catalog-empty">
                لا توجد مواعيد منشورة لهذا الخيار حاليًا.
              </div>
            ) : (
              visibleSchedules.map((schedule) => (
                <article
                  className="training-schedule-card"
                  key={schedule.id}
                >
                  <div>
                    <strong>
                  ...
                  <div>
                    <Link
                      href={`/training-booking?id=${encodeURIComponent(
                        schedule.id,
                      )}`}
